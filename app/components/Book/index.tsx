...
export default function Book(props: IProps) {
    return (
        <div>
            ...
            {
                props.type === 'Content' && props.component ? <ul>{props.component()}</ul> :
                    <ul className={`flip_book list-none w-80 h-100 duration-1500 transform-3d perspective-[900px] hover:translate-x-[${props.hoverMoveX || 0}]`}>
                        {
                            props.dataList.map((item, index) => {
                                ...
                                return <li className={`absolute w-full h-full origin-left -rotate-y-${25 - index * 2} duration-${2000 - 300 * index}`} key={index}><Image className='w-full h-full shadow-black' alt={`第${index + 1}页`} src={item} /></li>
